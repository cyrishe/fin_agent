...
按用户要求，标准Skill分析的目录/查询/明细调用上限由12/8/6调整为16/12/8。普通取数、fast、data-only、单次输出及超时保持原样。服务器.env未显式覆盖这三个Skill字段，无需改配置；仍需执行下方完整重启加载后端。

本地DSH协议57项、Python运行时与执行模式43项通过。新增回归确认提高上限不改变模型可见引导，成功取数后不因剩余额度强制追加模型调用，显式配置覆盖继续有效。这是机制测试，不宣称模型绝不会冗余取数；未为此次上限调整新增真实模型评测。
