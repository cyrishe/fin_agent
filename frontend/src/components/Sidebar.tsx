import { MessageSquarePlus, PanelLeftClose, Search, Sparkles } from "lucide-react";
...
export default function Sidebar(props: Props) {
  ...
  return (
    <aside className="sidebar-panel">
      <div className="brand-row"><div className="brand-mark"><Sparkles size={19} /></div><div><strong>Fin Agent</strong><span>金融智能工作台</span></div>{props.onClose && <button className="icon-button sidebar-close" onClick={props.onClose} aria-label="关闭会话列表"><PanelLeftClose size={18} /></button>}</div>
      ...
      <div className="sidebar-footer"><span className="online-dot" />默认访客账户</div>
    </aside>
  );
}
