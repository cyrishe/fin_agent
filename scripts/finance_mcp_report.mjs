...
for (const [i, spec] of specs.entries()) {
  ...
  spec.rows.forEach((row, j) => sheet.getRange(`A${j+7}:${end}${j+7}`).format.rowHeight =
    Math.min(409, Math.max(48, ...row.map((v,k) => lines(v, spec.widths[k])*15+12))));
  ...
  if (last > 6) sheet.tables.add(`A6:${end}${last}`, true, `McpResults${i}`);
}
wb.recalculate();
...
if (process.env.FINANCE_EVAL_RENDER === '1') {
  for (const [i, spec] of specs.entries()) {
    const preview = await wb.render({sheetName:spec.name, range: i === 0 ? 'A6:G9' : 'A6:F8', scale:1, format:'png'});
