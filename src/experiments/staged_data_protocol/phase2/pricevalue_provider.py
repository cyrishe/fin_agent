from __future__ import annotations

import re
from dataclasses import dataclass
from datetime import date, datetime
from decimal import Decimal
from typing import Any, Dict, List, Mapping

import pymysql

from src.experiments.staged_data_protocol.phase2.quote_provider import DEFAULT_MARKET_CODE, TRADE_CALENDAR_TABLE
from src.utils.mysql_utils import StockInfoDbUtils


@dataclass(frozen=True)
class PricevalueSource:
    subject: str
    table: str
    base_table: str
    join_on: str
    fields: Mapping[str, str]


PRICEVALUE_SOURCES: Dict[str, PricevalueSource] = {
    "stock": PricevalueSource(
        subject="stock",
        table="kcrp_stock_pricevaluate",
        base_table="kcrp_stock_baseinfo",
        join_on="b.stk_code = q.stk_code",
        fields={
            "code": "q.stk_code",
            "name": "b.stk_name",
            "tradedate": "q.trade_date",
            "market_value": "q.total_mv",
            "free_float_mv": "q.free_float_mv",
            "float_mv": "q.float_mv",
            "pe": "q.pe_ttm",
            "pe_lyr": "q.pe_lyr",
            "pe_ttm": "q.pe_ttm",
            "pe_ttm_deduct": "q.pe_ttm_deduct",
            "pe_dynamic": "q.pe_dynamic",
            "pb": "q.pb_mrq",
            "pb_mrq": "q.pb_mrq",
            "pb_lf": "q.pb_lf",
            "ps": "q.ps_ttm",
            "ps_lyr": "q.ps_lyr",
            "ps_ttm": "q.ps_ttm",
            "pcf_cfottm": "q.pcf_cfottm",
            "pcf_ncfttm": "q.pcf_ncfttm",
            "pcf_cfolyr": "q.pcf_cfolyr",
            "pcf_ncflyr": "q.pcf_ncflyr",
            "prr_lyr": "q.prr_lyr",
            "total_share": "q.total_share",
            "float_share": "q.float_share",
            "turn_ratio": "q.turn_ratio",
            "dividend_yield": "NULL",
        },
    ),
    "plate": PricevalueSource(
        subject="plate",
        table="kcrp_yp_plate_pricevaluate",
        base_table="kcrp_yp_plate",
        join_on="b.plate_code = q.plate_code",
        fields={
            "code": "q.plate_code",
            "name": "q.plate_name",
            "plate_code": "q.plate_code",
            "plate_name": "q.plate_name",
            "tradedate": "q.trade_date",
            "pe": "q.pe_ttm",
            "pe_lyr": "q.pe_lyr",
            "pe_ttm": "q.pe_ttm",
            "pb": "q.pb",
            "ps": "q.ps_lyr",
            "ps_lyr": "q.ps_lyr",
            "pcf_lyr": "q.pcf_lyr",
        },
    ),
    "index": PricevalueSource(
        subject="index",
        table="kcrp_index_pricevaluate",
        base_table="kcrp_index_base",
        join_on="b.idx_code = q.idx_code",
        fields={
            "code": "q.idx_code",
            "name": "q.index_short_name",
            "tradedate": "q.trade_date",
            "market_value": "q.total_mv",
            "float_mv": "q.float_mv",
            "pe": "q.pe_ttm",
            "pe_lyr": "q.pe_lyr",
            "pe_ttm": "q.pe_ttm",
            "pb": "q.pb_mrq",
            "pb_mrq": "q.pb_mrq",
            "ps": "q.ps_ttm",
            "ps_lyr": "q.ps_lyr",
            "ps_ttm": "q.ps_ttm",
            "pcf_lyr": "q.pcf_lyr",
            "pcf_ttm": "q.pcf_ttm",
            "dividend_yield": "NULL",
            "pe_percentile": "NULL",
        },
    ),
}


MOCK_PRICEVALUE_FIELDS: Dict[str, List[str]] = {
    "industry": ["industry_code", "industry_name", "pe", "pb"],
    "fund": ["code", "name", "nav", "premium"],
}


OP_SQL = {"=": "=", "==": "=", "!=": "!=", ">": ">", ">=": ">=", "<": "<", "<=": "<="}
FILTER_RE = re.compile(
    r"(?:(?P<connector>\band\b|\bor\b)\s+)?"
    r"(?P<field>[A-Za-z_]\w*)\s*"
    r"(?P<op>in|=|==|!=|>=|<=|>|<)\s*"
    r"(?P<value>\[[^\]]+\]|\([^)]+\)|[^,;]+?)"
    r"(?=\s+(?:and|or)\s+[A-Za-z_]\w*\s*(?:in|=|==|!=|>=|<=|>|<)|[,;]|$)",
    flags=re.IGNORECASE,
)


def execute_pricevalue_api(*, subject: str, args: Mapping[str, Any], outputs: List[str]) -> Dict[str, Any]:
    source = PRICEVALUE_SOURCES.get(subject) or _mock_source(subject)
    columns = _requested_columns(source=source, outputs=outputs)
    ignored_filters = _ignored_filters(source=source, args=args)
    if not source.table:
        return _standard_result(
            status="ok",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            mocked_fields=columns,
            ignored_filters=ignored_filters,
        )
    if _has_unresolved_ref(args):
        return _standard_result(
            status="requires_upstream_values",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            mocked_fields=_mocked_fields(source=source, columns=columns),
            ignored_filters=ignored_filters,
            reason="filter contains result references that must be materialized by the execution engine",
        )

    limit = _bounded_limit(args.get("limit"))
    where_sql, params = _build_where(source=source, args=args)
    order_sql = _build_order(source=source, args=args)
    sql = _build_sql(source=source, columns=columns, where_sql=where_sql, order_sql=order_sql)
    params.append(limit)

    db = StockInfoDbUtils(database="kingdomai")
    try:
        with db.conn.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute(sql, tuple(params))
            raw_rows = cursor.fetchall()
        rows = [_normalize_row(row, columns) for row in raw_rows]
        return _standard_result(
            status="ok",
            source=source,
            args=args,
            columns=columns,
            rows=rows,
            mocked_fields=_mocked_fields(source=source, columns=columns),
            ignored_filters=ignored_filters,
            sql_shape={"where": where_sql, "order": order_sql, "limit": limit},
        )
    except Exception as exc:  # noqa: BLE001 - experiment boundary should return structured failures.
        return _standard_result(
            status="provider_error",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            mocked_fields=_mocked_fields(source=source, columns=columns),
            ignored_filters=ignored_filters,
            reason=str(exc),
        )
    finally:
        db.close_db()


def execute_kd_pricevalue_api(
    *,
    subject: str,
    field: str,
    method: str,
    args: Mapping[str, Any],
    outputs: List[str],
) -> Dict[str, Any]:
    source = PRICEVALUE_SOURCES.get(subject) or _mock_source(subject)
    api_name = f"{subject}.pricevalue.kd_{field}_{method}"
    columns = _requested_kd_columns(outputs)
    if not source.table or field not in source.fields or source.fields[field] == "NULL":
        return _standard_result(
            status="unsupported",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            api_name=api_name,
            mocked_fields=columns,
            reason="kd pricevalue provider is only available for real pricevalue fields",
        )
    if method == "percentile" and subject != "stock":
        return _standard_result(
            status="unsupported",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            api_name=api_name,
            mocked_fields=columns,
            reason="pricevalue percentile is currently implemented for stock only",
        )
    if method not in {"sum", "avg", "max", "min", "median", "count", "percentile"}:
        return _standard_result(
            status="unsupported",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            api_name=api_name,
            mocked_fields=columns,
            reason=f"method={method} is not supported for pricevalue field={field}",
        )
    if _has_unresolved_ref(args):
        return _standard_result(
            status="requires_upstream_values",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            api_name=api_name,
            mocked_fields=[],
            reason="filter contains result references that must be materialized by the execution engine",
        )

    k = _bounded_k(args.get("k") or args.get("window"))
    limit = _bounded_limit(args.get("limit"))
    identity_sql, identity_params = _build_identity_where(source=source, args=args)
    sql = (
        _build_kd_percentile_sql(source=source, field=field, identity_sql=identity_sql)
        if method == "percentile"
        else _build_kd_sql(source=source, field=field, method=method, identity_sql=identity_sql)
    )
    market_code = _calendar_market_code(args)
    as_of = _calendar_as_of(args)
    params = [market_code, market_code, as_of, k, *identity_params]

    db = StockInfoDbUtils(database="kingdomai")
    try:
        with db.conn.cursor(pymysql.cursors.DictCursor) as cursor:
            cursor.execute(sql, tuple(params))
            raw_rows = cursor.fetchall()
        metric_rows = [_normalize_kd_metric_row(row, source=source, field=field, method=method, k=k) for row in raw_rows]
        metric_rows = _filter_kd_rows(metric_rows, args=args, field=field)
        metric_rows = _sort_kd_rows(metric_rows, args=args)
        if limit > 0:
            metric_rows = metric_rows[:limit]
        rows = [_project_kd_row(row, outputs=outputs, columns=columns) for row in metric_rows]
        return _standard_result(
            status="ok",
            source=source,
            args=args,
            columns=columns,
            rows=rows,
            api_name=api_name,
            mocked_fields=[],
            sql_shape={
                "calendar": TRADE_CALENDAR_TABLE,
                "market_code": market_code,
                "window": "latest k trade dates ending at latest trade day before today",
                "identity_filter": identity_sql,
                "field": field,
                "method": method,
                "k": k,
                "limit": limit,
            },
        )
    except Exception as exc:  # noqa: BLE001 - experiment boundary should return structured failures.
        return _standard_result(
            status="provider_error",
            source=source,
            args=args,
            columns=columns,
            rows=[],
            api_name=api_name,
            mocked_fields=[],
            reason=str(exc),
        )
    finally:
        db.close_db()


def _mock_source(subject: str) -> PricevalueSource:
    fields = {field: "NULL" for field in MOCK_PRICEVALUE_FIELDS.get(subject, ["code", "name", "pe", "pb", "ps", "market_value"])}
    return PricevalueSource(subject=subject, table="", base_table="", join_on="", fields=fields)


def _requested_columns(*, source: PricevalueSource, outputs: List[str]) -> List[str]:
    columns: List[str] = []
    for output in outputs:
        token = _output_token(output)
        if token in source.fields and token not in columns:
            columns.append(token)
    if columns:
        return columns
    defaults = {
        "industry": ["industry_code", "industry_name", "pe", "pb"],
        "fund": ["code", "name", "nav", "premium"],
        "plate": ["plate_code", "plate_name", "tradedate", "pe", "pb"],
        "index": ["code", "name", "tradedate", "pe", "pb", "dividend_yield", "pe_percentile"],
    }
    return [field for field in defaults.get(source.subject, ["code", "name", "tradedate", "pe", "pb", "ps", "market_value"]) if field in source.fields]


def _output_token(output: str) -> str:
    text = str(output or "").strip()
    if " as " in text:
        text = text.split(" as ", 1)[0].strip()
    if "." in text:
        text = text.rsplit(".", 1)[-1]
    return text


def _has_unresolved_ref(args: Mapping[str, Any]) -> bool:
    return any(isinstance(value, str) and re.search(r"\br\d+\.", value) for value in args.values())


def _bounded_limit(value: Any) -> int:
    try:
        parsed = int(value)
    except Exception:
        parsed = 100
    if parsed <= 0:
        parsed = 100
    return max(1, min(parsed, 500))


def _bounded_k(value: Any) -> int:
    text = str(value or "").strip().lower()
    if text.endswith("y"):
        try:
            parsed = int(float(text[:-1]) * 250)
        except ValueError:
            parsed = 250
    elif text.endswith("m"):
        try:
            parsed = int(float(text[:-1]) * 21)
        except ValueError:
            parsed = 21
    elif text.endswith("d"):
        try:
            parsed = int(float(text[:-1]))
        except ValueError:
            parsed = 250
    else:
        try:
            parsed = int(value)
        except Exception:
            parsed = 250
    return max(1, min(parsed, 2500))


def _calendar_market_code(args: Mapping[str, Any]) -> str:
    return str(args.get("market_code") or args.get("calendar_market_code") or DEFAULT_MARKET_CODE)


def _calendar_as_of(args: Mapping[str, Any]) -> str:
    value = str(args.get("as_of") or "").strip()
    return value if value and value != "-1" else date.today().isoformat()


def _build_where(*, source: PricevalueSource, args: Mapping[str, Any]) -> tuple[str, List[Any]]:
    clauses: List[str] = []
    params: List[Any] = []
    exact_date = str(args.get("date") or args.get("tradedate") or "").strip()
    start = str(args.get("start") or args.get("start_date") or "").strip()
    end = str(args.get("end") or args.get("end_date") or "").strip()
    if exact_date:
        clauses.append(f"{source.fields['tradedate']} = %s")
        params.append(exact_date)
    elif start and end:
        clauses.append(f"{source.fields['tradedate']} BETWEEN %s AND %s")
        params.extend(sorted([start, end]))
    else:
        clauses.append(f"{source.fields['tradedate']} = (SELECT MAX(trade_date) FROM {source.table})")

    filter_sql, filter_params = _build_filter_clauses(source=source, args=args)
    if filter_sql:
        clauses.append(filter_sql)
        params.extend(filter_params)
    return " AND ".join(clauses), params


def _build_identity_where(*, source: PricevalueSource, args: Mapping[str, Any]) -> tuple[str, List[Any]]:
    clauses: List[str] = []
    params: List[Any] = []
    for connector, field_name, op, value in _explicit_filters(source=source, args=args):
        if field_name not in {"code", "name"}:
            continue
        expression = source.fields.get(field_name)
        if not expression or expression == "NULL":
            continue
        prefix = connector if clauses else ""
        if op == "in":
            values = _list_value(value)
            if not values:
                continue
            placeholders = ", ".join(["%s"] * len(values))
            clauses.append(f"{prefix} {expression} IN ({placeholders})".strip())
            params.extend(values)
            continue
        if op in {"=", "=="}:
            clauses.append(f"{prefix} {expression} = %s".strip())
            params.append(value)
    if not clauses:
        return "", []
    return f"AND ({' '.join(clauses)})", params


def _build_filter_clauses(*, source: PricevalueSource, args: Mapping[str, Any]) -> tuple[str, List[Any]]:
    clauses: List[str] = []
    params: List[Any] = []
    for connector, field_name, op, value in _explicit_filters(source=source, args=args):
        expression = source.fields.get(field_name)
        if not expression or expression == "NULL":
            continue
        prefix = connector if clauses else ""
        if op == "in":
            values = _list_value(value)
            if not values:
                continue
            placeholders = ", ".join(["%s"] * len(values))
            clauses.append(f"{prefix} {expression} IN ({placeholders})".strip())
            params.extend(values)
            continue
        if op not in OP_SQL:
            continue
        clauses.append(f"{prefix} {expression} {OP_SQL[op]} %s".strip())
        params.append(value)
    if not clauses:
        return "", []
    return f"({' '.join(clauses)})", params


def _ignored_filters(*, source: PricevalueSource, args: Mapping[str, Any]) -> List[str]:
    ignored: List[str] = []
    for _connector, field_name, op, value in _explicit_filters(source=source, args=args):
        if source.fields.get(field_name) == "NULL":
            ignored.append(f"{field_name} {op} {value}")
    return ignored


def _explicit_filters(*, source: PricevalueSource, args: Mapping[str, Any]) -> List[tuple[str, str, str, Any]]:
    rows: List[tuple[str, str, str, Any]] = []
    for field_name in source.fields:
        value = args.get(field_name)
        if value not in (None, ""):
            rows.append(("AND", field_name, "=", value))
    for plural, field_name in [("codes", "code"), ("names", "name")]:
        value = args.get(plural)
        if value not in (None, "") and field_name in source.fields:
            rows.append(("AND", field_name, "in", value))
    filter_text = str(args.get("filter") or "").strip()
    for match in FILTER_RE.finditer(filter_text):
        field_name = _normalize_filter_field(
            source=source,
            field_name=str(match.group("field") or "").strip(),
        )
        rows.append(
            (
                str(match.group("connector") or "AND").upper(),
                field_name,
                str(match.group("op") or "").strip().lower(),
                _clean_value(str(match.group("value") or "")),
            )
        )
    return rows


def _normalize_filter_field(*, source: PricevalueSource, field_name: str) -> str:
    if source.subject == "stock":
        aliases = {
            "stock_code": "code",
            "stk_code": "code",
            "stock_name": "name",
            "stk_name": "name",
        }
        return aliases.get(field_name, field_name)
    return field_name


def _clean_value(value: str) -> Any:
    text = str(value or "").strip().strip("\"'")
    try:
        if "." in text:
            return float(text)
        return int(text)
    except ValueError:
        return text


def _list_value(value: Any) -> List[Any]:
    if isinstance(value, (list, tuple, set)):
        return list(value)
    text = str(value or "").strip()
    if not text:
        return []
    if (text.startswith("[") and text.endswith("]")) or (text.startswith("(") and text.endswith(")")):
        text = text[1:-1]
    return [_clean_value(item) for item in text.split(",") if str(item).strip()]


def _build_order(*, source: PricevalueSource, args: Mapping[str, Any]) -> str:
    text = str(args.get("order") or "").strip()
    if not text or text.lower() == "none":
        return f"{source.fields['tradedate']} DESC"
    parts = re.split(r"\s+", text, maxsplit=1)
    field_name = parts[0].strip()
    direction = parts[1].strip().lower() if len(parts) > 1 else "desc"
    expression = source.fields.get(field_name)
    if not expression or expression == "NULL":
        return f"{source.fields['tradedate']} DESC"
    direction_sql = "ASC" if direction.startswith("asc") else "DESC"
    return f"{expression} {direction_sql}, {source.fields['tradedate']} DESC"


def _build_sql(*, source: PricevalueSource, columns: List[str], where_sql: str, order_sql: str) -> str:
    select_sql = ", ".join(f"{source.fields[column]} AS `{column}`" for column in columns)
    return f"""
        SELECT {select_sql}
        FROM {source.table} q
        LEFT JOIN {source.base_table} b ON {source.join_on}
        WHERE {where_sql}
        ORDER BY {order_sql}
        LIMIT %s
    """


def _build_kd_sql(*, source: PricevalueSource, field: str, method: str, identity_sql: str) -> str:
    if method == "median":
        return _build_kd_median_sql(source=source, field=field, identity_sql=identity_sql)
    agg = {"max": "MAX", "min": "MIN", "avg": "AVG", "sum": "SUM", "count": "COUNT"}.get(method) or "SUM"
    expression = source.fields[field]
    return f"""
        WITH kd_dates AS (
            SELECT calendar_date AS trade_date
            FROM {TRADE_CALENDAR_TABLE}
            WHERE market_code = %s
              AND is_trade_day = 1
              AND trade_seq <= (
                  SELECT trade_seq
                  FROM {TRADE_CALENDAR_TABLE}
                  WHERE market_code = %s
                    AND is_trade_day = 1
                    AND calendar_date < %s
                  ORDER BY calendar_date DESC
                  LIMIT 1
              )
            ORDER BY trade_seq DESC
            LIMIT %s
        )
        SELECT
            {source.fields['code']} AS `code`,
            {source.fields['name']} AS `name`,
            {agg}({expression}) AS `value`,
            MAX({source.fields['tradedate']}) AS `end_date`,
            COUNT({expression}) AS `window_count`
        FROM {source.table} q
        LEFT JOIN {source.base_table} b ON {source.join_on}
        WHERE {source.fields['tradedate']} IN (SELECT trade_date FROM kd_dates)
          AND {expression} IS NOT NULL
          {identity_sql}
        GROUP BY {source.fields['code']}, {source.fields['name']}
        HAVING `value` IS NOT NULL
    """


def _build_kd_median_sql(*, source: PricevalueSource, field: str, identity_sql: str) -> str:
    expression = source.fields[field]
    return f"""
        WITH kd_dates AS (
            SELECT calendar_date AS trade_date
            FROM {TRADE_CALENDAR_TABLE}
            WHERE market_code = %s
              AND is_trade_day = 1
              AND trade_seq <= (
                  SELECT trade_seq
                  FROM {TRADE_CALENDAR_TABLE}
                  WHERE market_code = %s
                    AND is_trade_day = 1
                    AND calendar_date < %s
                  ORDER BY calendar_date DESC
                  LIMIT 1
              )
            ORDER BY trade_seq DESC
            LIMIT %s
        )
        SELECT
            code,
            name,
            AVG(metric_value) AS `value`,
            MAX(end_date) AS `end_date`,
            MAX(cnt) AS `window_count`
        FROM (
            SELECT
                base_rows.*,
                ROW_NUMBER() OVER (PARTITION BY code ORDER BY metric_value) AS rn,
                COUNT(*) OVER (PARTITION BY code) AS cnt,
                MAX(tradedate) OVER (PARTITION BY code) AS end_date
            FROM (
                SELECT
                    {source.fields['code']} AS `code`,
                    {source.fields['name']} AS `name`,
                    {source.fields['tradedate']} AS `tradedate`,
                    {expression} AS `metric_value`
                FROM {source.table} q
                LEFT JOIN {source.base_table} b ON {source.join_on}
                WHERE {source.fields['tradedate']} IN (SELECT trade_date FROM kd_dates)
                  AND {expression} IS NOT NULL
                  {identity_sql}
            ) base_rows
        ) ranked
        WHERE rn IN (FLOOR((cnt + 1) / 2), FLOOR((cnt + 2) / 2))
        GROUP BY code, name
        HAVING `value` IS NOT NULL
    """


def _build_kd_percentile_sql(*, source: PricevalueSource, field: str, identity_sql: str) -> str:
    expression = source.fields[field]
    return f"""
        WITH kd_dates AS (
            SELECT calendar_date AS trade_date
            FROM {TRADE_CALENDAR_TABLE}
            WHERE market_code = %s
              AND is_trade_day = 1
              AND trade_seq <= (
                  SELECT trade_seq
                  FROM {TRADE_CALENDAR_TABLE}
                  WHERE market_code = %s
                    AND is_trade_day = 1
                    AND calendar_date < %s
                  ORDER BY calendar_date DESC
                  LIMIT 1
              )
            ORDER BY trade_seq DESC
            LIMIT %s
        ),
        current_trade_day AS (
            SELECT MAX(trade_date) AS trade_date
            FROM kd_dates
        ),
        current_values AS (
            SELECT
                {source.fields['code']} AS code,
                {source.fields['name']} AS name,
                {source.fields['tradedate']} AS tradedate,
                {expression} AS current_value
            FROM {source.table} q
            LEFT JOIN {source.base_table} b ON {source.join_on}
            WHERE {source.fields['tradedate']} = (SELECT trade_date FROM current_trade_day)
              AND {expression} IS NOT NULL
              {identity_sql}
        ),
        window_values AS (
            SELECT
                {source.fields['code']} AS code,
                {expression} AS metric_value
            FROM {source.table} q
            JOIN current_values cv ON {source.fields['code']} = cv.code
            WHERE {source.fields['tradedate']} IN (SELECT trade_date FROM kd_dates)
              AND {expression} IS NOT NULL
        )
        SELECT
            c.code,
            c.name,
            c.tradedate AS end_date,
            c.current_value,
            COUNT(w.metric_value) AS window_count,
            SUM(CASE WHEN w.metric_value <= c.current_value THEN 1 ELSE 0 END) / COUNT(w.metric_value) AS value
        FROM current_values c
        JOIN window_values w ON w.code = c.code
        GROUP BY c.code, c.name, c.tradedate, c.current_value
    """


def _normalize_kd_metric_row(row: Mapping[str, Any], *, source: PricevalueSource, field: str, method: str, k: int) -> Dict[str, Any]:
    current_value = _normalize_value(row.get("current_value"))
    return {
        "code": str(row.get("code") or "").strip(),
        "name": row.get("name"),
        field: current_value,
        "current_value": current_value,
        "value": _normalize_value(row.get("value")),
        "k": k,
        "end_date": _normalize_value(row.get("end_date")),
        "tradedate": _normalize_value(row.get("end_date")),
        "window_count": int(row.get("window_count") or 0),
        "method": f"kd_{field}_{method}",
        "source": source.table,
    }


def _filter_kd_rows(rows: List[Dict[str, Any]], *, args: Mapping[str, Any], field: str) -> List[Dict[str, Any]]:
    allowed = {"value", "k", "end_date", "tradedate", "current_value", field}
    filters = [item for item in _explicit_filters(source=_filter_source(allowed), args=args) if item[1] in allowed]
    result = rows
    for _connector, field_name, op, expected in filters:
        if op not in OP_SQL:
            continue
        result = [row for row in result if _compare(row.get(field_name), op, expected)]
    return result


def _filter_source(fields: set[str]) -> PricevalueSource:
    return PricevalueSource(subject="stock", table="", base_table="", join_on="", fields={field: field for field in fields})


def _compare(left: Any, op: str, right: Any) -> bool:
    try:
        left_value: Any = float(left)
        right_value: Any = float(right)
    except (TypeError, ValueError):
        left_value = str(left or "")
        right_value = str(right or "")
    if op in {"=", "=="}:
        return left_value == right_value
    if op == "!=":
        return left_value != right_value
    if op == ">":
        return left_value > right_value
    if op == ">=":
        return left_value >= right_value
    if op == "<":
        return left_value < right_value
    if op == "<=":
        return left_value <= right_value
    return False


def _sort_kd_rows(rows: List[Dict[str, Any]], *, args: Mapping[str, Any]) -> List[Dict[str, Any]]:
    text = str(args.get("order") or "").strip()
    if not text or text.lower() == "none":
        return sorted(rows, key=lambda row: str(row.get("code") or ""))
    parts = re.split(r"\s+", text, maxsplit=1)
    field_name = parts[0].strip()
    direction = parts[1].strip().lower() if len(parts) > 1 else "asc"
    reverse = not direction.startswith("asc")
    if field_name not in {"code", "name", "value", "k", "end_date", "tradedate", "current_value"}:
        field_name = "value"
    return sorted(rows, key=lambda row: (row.get(field_name) is None, row.get(field_name)), reverse=reverse)


def _requested_kd_columns(outputs: List[str]) -> List[str]:
    columns: List[str] = []
    for output in outputs:
        name = _output_column_name(output)
        if name and name not in columns:
            columns.append(name)
    return columns or ["code", "name", "value"]


def _project_kd_row(row: Mapping[str, Any], *, outputs: List[str], columns: List[str]) -> Dict[str, Any]:
    projected: Dict[str, Any] = {}
    for output, column in zip(outputs, columns):
        token = _output_token(output)
        projected[column] = _normalize_value(row.get(token))
    return projected


def _output_column_name(output: str) -> str:
    text = str(output or "").strip()
    if " as " in text:
        return text.split(" as ", 1)[1].strip()
    if "." in text:
        text = text.rsplit(".", 1)[-1]
    return text


def _normalize_row(row: Mapping[str, Any], columns: List[str]) -> Dict[str, Any]:
    return {column: _normalize_value(row.get(column)) for column in columns}


def _normalize_value(value: Any) -> Any:
    if isinstance(value, Decimal):
        return float(value)
    if isinstance(value, (datetime, date)):
        return value.isoformat()
    return value


def _mocked_fields(*, source: PricevalueSource, columns: List[str]) -> List[str]:
    return [column for column in columns if source.fields.get(column) == "NULL"]


def _standard_result(
    *,
    status: str,
    source: PricevalueSource,
    args: Mapping[str, Any],
    columns: List[str],
    rows: List[Dict[str, Any]],
    api_name: str | None = None,
    mocked_fields: List[str] | None = None,
    ignored_filters: List[str] | None = None,
    reason: str | None = None,
    sql_shape: Mapping[str, Any] | None = None,
) -> Dict[str, Any]:
    result: Dict[str, Any] = {
        "status": status,
        "api": api_name or f"{source.subject}.pricevalue",
        "subject": source.subject,
        "table": source.table or None,
        "arguments": dict(args),
        "columns": columns,
        "rows": rows,
        "mocked_fields": mocked_fields or [],
        "ignored_filters": ignored_filters or [],
    }
    if reason:
        result["reason"] = reason
    if sql_shape:
        result["sql_shape"] = dict(sql_shape)
    return result
