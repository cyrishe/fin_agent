...
> 2026-09-06 最新状态：迁移已完成，本地与服务器 `SYSTEM_DB_URL` 已指向新 stock_agent。
> 21 张源表、17,386 条记录校验通过，新增 3 张应用表；每日用量接口已验证。
> 详细结果及剩余启动步骤见 [迁移报告](system_db_cutover_20260906.md)。下文保留迁移前核查和操作顺序作为历史说明，不要重复执行复制。
...
2026-09-06迁移前核查：本地和服务器均连接 `47.112.132.214:3306/aiia_system`，
...
迁移前 Fin Agent 的本地、服务器配置及服务器进程指向旧 `aiia_system`。
...
当时预检本身没有复制数据或切换 `.env`；后续停写、复制、校验、切换已完成，见文首结果。
